...
class HeroWinrateSpider(scrapy.Spider):
            ...
            for time in self.selects['time']:
                for server in self.selects['server']:
                    for url in db.lrange('herourls', 0, 999):
                        ...
                        self.start_urls.append(
                            'http://dotamax.com/' + url.split('/')[1] +
                            '/detail/match_up_comb/' + url.split('/')[3] +
                            '?time=' + time + '&server=' + server +
                            '&skill=' + skill
                        )

    def parse(self, response):
        matchup = response.css('.sortable tr td:nth-child(2) ' +
                               'div:first-child').re(r'\">([0-9\.\-]+)%</div>')
        opponent_names = response.css(
            '.hero-name-list').re(r'list\">([a-zA-Z \-\']+)</span>')
        name = response.css(
            '.hero-title').re(r'\">\n *([a-zA-Z \-\']+?) *</span>')[0]

        select = re.match(
            r'.*match\_up\_(.*?)\/.*\?time\=(.*)\&server\=(.*)\&skill\=(.*)',
            response.url
        )
        self.save_winrate(
            matchup, opponent_names, name,
            select.group(1)[0] + select.group(2)[0] +
            select.group(3)[0] + select.group(4)[0]
        )
    ...
    def updateDate(self):
        db = redis.StrictRedis(db=1)
        db.set('dphw_update_time', Time.ctime())
